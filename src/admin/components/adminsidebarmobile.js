    ...
    const MenuItem = React.memo(({ item, index }) => {
        ...
        return (
            <motion.div
                ...
                transition={{ delay: hasAnimated ? 0 : index * 0.05 }}
                className="relative"
                layout={false}
                onAnimationComplete={() => {
            ...
            >
                ...
                >
                    ...
                    <FiChevronRight className={`w-4 h-4 ${colors.icon} transition-transform duration-200 ${active ? 'rotate-90' : ''}`} />
                </motion.button>
            </motion.div>
        );
    });
        ...
        <AnimatePresence>
            {isSidebarOpen && (
                <motion.div
                    ...
                    className="fixed inset-y-0 left-0 w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 z-50 flex flex-col shadow-xl"
                >
                    <motion.div
